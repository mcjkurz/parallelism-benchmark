...
def check_artifacts_exist():
    ...
    required_dirs = ["tokenizer", "char_model", "coup_model", "poem4_model", "poem1_model"]
    missing = []
    ...
    for dirname in required_dirs:
        path = os.path.join(ARTIFACTS_DIR, dirname)
        if not os.path.exists(path):
            missing.append(dirname)
    
    if missing:
        print(f"Error: Missing model artifacts in {ARTIFACTS_DIR}:")
        for m in missing:
            print(f"  - {m}")
        print()
        print("Please run the training pipeline first:")
...
def main():
    ...
    char_model = BertForSequenceClassification.from_pretrained(os.path.join(ARTIFACTS_DIR, "char_model")).to(device)
    coup_model = BertForSequenceClassification.from_pretrained(os.path.join(ARTIFACTS_DIR, "coup_model")).to(device)
    poem4_model = PoemParallelismClassifier.from_pretrained(os.path.join(ARTIFACTS_DIR, "poem4_model")).to(device)
    poem1_model = BertForSequenceClassification.from_pretrained(os.path.join(ARTIFACTS_DIR, "poem1_model")).to(device)
